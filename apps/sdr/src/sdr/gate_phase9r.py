"""Phase 9R targeted live-LLM gate — G1–G6 recovery cases.

Usage (from apps/sdr, with OPENAI_API_KEY in .env):

    uv run python -m sdr.gate_phase9r
...
PHASE9_DIR = ROOT / "tests" / "golden" / "phase9"
PHASE9R_DIR = ROOT / "tests" / "golden" / "phase9r"
OUT_DIR = ROOT / ".gate" / "phase9r"
TZ_BRT = ZoneInfo("America/Sao_Paulo")

# gate_id, relative path from apps/sdr, repeats
GATE_CASES: tuple[tuple[str, Path, int], ...] = (
    ("G1", PHASE9_DIR / "p9_03_cumprimento_seguido_de_imagem.json", 3),
    ("G2", PHASE9R_DIR / "g2_reply_strada_financia.json", 3),
    ("G3", PHASE9_DIR / "p9_15_item_sem_informacao_segura.json", 3),
    ("G4", PHASE9_DIR / "p9_10_todos_documentos.json", 2),
    ("G5", PHASE9R_DIR / "g5_cnh_resto_na_loja.json", 2),
    ("G6a", PHASE9R_DIR / "g6_financia_100.json", 1),
    ("G6b", PHASE9R_DIR / "g6_entao_aprovado.json", 1),
    ("G6c", PHASE9R_DIR / "g6_qual_taxa.json", 1),
    ("G6d", PHASE9R_DIR / "g6_parcela_1500.json", 1),
)
...
def main() -> None:
    ...
    rid = datetime.now(TZ_BRT).strftime("phase9r-%Y%m%dT%H%M%S")
    dest = OUT_DIR / rid
    ...
    matrix_lines = [
        "# Fase 9R — matriz comercial (PENDING_HUMAN_REVIEW)",
        "",
